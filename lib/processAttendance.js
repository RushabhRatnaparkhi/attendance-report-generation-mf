/**
 * Process attendance data from CSV format using JCL SORT logic
 * Expected format: EmpID,Name,Date,Status
 * or: EmpID,Name,Date,Status,CheckIn,CheckOut
 * 
 * JCL SORT SIMULATION:
 * 1. Remove duplicates based on EmpID + Date (primary key)
 * 2. Sort by EmpID (ascending) - simulating JCL SORT FIELDS=(1,8,CH,A)
 * 3. Secondary sort by Date (ascending)
 */
...
export function processAttendanceCSV(fileContent) {
  ...
  // Step 1: Parse and remove duplicates
  for (const line of dataLines) {
    ...
    // Create unique key for duplicate detection (like primary key in mainframe)
  ...
  // Step 2: JCL SORT - Sort by EmpID (primary) and Date (secondary)
  // Similar to: SORT FIELDS=(1,8,CH,A,9,10,CH,A)
  records.sort((a, b) => {
    // Primary sort: EmpID (ascending)
  ...
  console.log(`📊 JCL SORT Applied to Attendance Data:
    - Total records processed: ${dataLines.length}
    - Duplicates removed: ${duplicates}
    - Unique records after sort: ${records.length}
    - Sorted by: EmpID (primary), Date (secondary)
  `);
